...
Note::Note(const vector<unsigned int>& dataInfo, const vector<string>& studentInfo, const double waterNumber){
    setData(dataInfo);
...
void Note::setStudent(const vector<string>& studentInfo){
    string course;
    if (studentInfo[3] < COURSE_MIN) {
        course = COURSE_MIN;
    }
    else if (studentInfo[3] > COURSE_MAX) {
        course = COURSE_MAX;
    }
